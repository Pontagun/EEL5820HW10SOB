...
#     wx, wy = sobel()
#
#     gradx = innerprod(image, wx)
#     grady = innerprod(image, wy)
#
#     grad = np.square((gradx * gradx) + (grady * grady))
#
#     shpx = np.shape(grad)[0]
#     shpy = np.shape(grad)[1]
#     #
#
...
#             grad[i] = 0
#
#     grad = np.reshape(grad, (shpx, shpy))
#
#
#     im = Image.fromarray(grad)
#     if im.mode != 'RGB':
